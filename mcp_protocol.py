"""Core MCP protocol logic for the skincare server: tool implementations,
schemas, and JSON-RPC 2.0 message handling.

Shared by every transport adapter (stdio in server.py, Streamable HTTP in
http_server.py) so the same tool logic runs locally or remotely -- only
how the JSON-RPC messages travel changes.
"""
...
def handle_message(message: dict[str, Any]) -> dict[str, Any] | None:
    """Handle one JSON-RPC 2.0 message and return the response object, or
    None when the message was a notification (no response expected)."""
